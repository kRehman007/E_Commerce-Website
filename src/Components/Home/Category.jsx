...
const Category = () => {
  const navigate=useNavigate();
    const Array=[
        {id:1,icon:<CiMobile4 />,name:'Phones'},{id:2,icon:<HiOutlineComputerDesktop />,name:'Computers'},
        {id:4,icon:<BsSmartwatch />,name:'SmartWatch'},{id:4,icon:<FiHeadphones />,name:'Headphones'},
        {id:5,icon:<MdOutlineCameraAlt />,name:'Camera'},{id:6,icon:<LuGamepad2 />,name:'Gaming'}
    ]
  return (
  ...
  <span  className='text-sm text-red-600 font-semibold'>Categories</span></div>
  ...
  <div className='grid grid-cols-3 gap-2 lg:grid-cols-6 mt-3 md:pt-2 cursor-pointer'>
    {Array.map((item,index)=>(
        <div key={index} className='flex flex-col space-y-2 w-[90px] h-[90px] md:w-[120px] md:h-[120px] border border-gray-400
...
export const SellingProducts=()=>{
  ...
  const {AddItemsToWish,GetDocumentData,isWish,setIsWish,RemoveItemsToWish}=useEcomContext()
  ...
  useEffect(()=>{
    const getData=async()=>{
     ...
     const MenImage = FlashData?.images.filter(item => item.category.includes('Men'))[0];
const SportsImage = FlashData?.images.filter(item => item.category.includes('Sports'))[0];
const ToysImage = FlashData?.images.filter(item => item.category.includes('Toys'))[0];
const WomenImage = FlashData?.images.filter(item => item.category.includes('Women'))[0];
...
setData(selectedImages);
  ...
  <div className='flex space-x-1 rounded-sm items-center'><div className='w-3 h-[30px] bg-red-600 border-none'></div>
  <span  className='text-sm text-red-600 font-semibold'>This Month</span></div>
  ...
  <div className='grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 pt-2   '>
    ...
    <Card className='w-[150px] h-[200px] md:w-[260px] md:h-[300px]' >
      <Card.Img variant="top" src={item.src} className='h-[100px] md:h-[200px] object-cover
    ...
    <CustomModal isOpen={isOpen} onClose={onClose} title={ItemList?.name} img={ItemList?.img}
    id={ItemList?.id} price={ItemList?.price} />
...
useEffect(()=>{
  ...
  return(
    ...
    <p className='text-3xl text-green-600 font-medium mb-2'>Categories</p>
    <h1 className='font-semibold text-4xl mb-3 '>Enhance Your<br />Music Experience</h1>
    <button className='bg-green-600 px-4 py-2 text-xl'>Buy Now</button>  
    </div>
    <img src={Msc} className='p-3' />
    </div>):(
    <div style={{backgroundImage:`url(${Ms})`,backgroundSize:'cover'}}
