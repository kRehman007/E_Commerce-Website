...
function Header() {
  ...
  const { setIsSideBar, search, setSearch, setScreen, isScreen } =
    useEcomContext();
    ...
    <>
      <div className="bg-gray-900 w-[100%] text-white p-3 hidden md:block">
        <div className="flex space-x-2 lg:justify-center md:justify-start md:px-7">
          <div>
            Summer sale for All Swin Suits And Free Express Delivery -{" "}
            <span className="text-sm">OFF 50%!</span>
          </div>
          <a
            href="#"
            className="font-bold"
      ...
      {/* Menu...... */}
      <div
        className="flex justify-between pl-3 pr-3 lg:pr-16    md:pl-11 pt-1 pb-1 md:pt-3 md:pb-3   items-center
     sticky top-0 left-0 right-0 bg-gray-300 z-30 text-black w-[100%]  "
      >
        {search ? (
          <div
            className=" flex relative left-[50%] translate-x-[-50%] space-x-4  items-center
          ...
          <>
            <h1 className="font-bold text-2xl cursor-pointer flex space-x-6 items-center ">
              {isScreen ? (
                ...
                <span className="pl-8" onClick={() => navigate("/")}>
                  Exclusive
                </span>
              )}
            </h1>
