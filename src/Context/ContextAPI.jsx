...
import {useCreateUserWithEmailAndPassword,useSignInWithEmailAndPassword,
...
export const ContextProvider=({children})=>{
  ...
  const [currentUser,setCurrentUser]=useState(JSON.parse(localStorage.getItem('Ecomuser-info')))
...
const AddItemsToCart=(id,title,qty=1,img,price=0)=>{
  ...
  if(res){alert('Items has already Added to Cart')}
    ...
    setCartItem(prev => [...prev,{id:id,title:title,qty:qty,img:img,price:price}])

  }
}

const UpdatePlusToCart=(item)=>{
  setCartItem(prev=>prev.map(cartItems=>(
...
const calculateTotalPrice = () => {
  setTotalPrice(cartItem?.reduce((total, item) => total + (item.price*item.qty), 0))
}
...
const isLoggedIn=currentUser? true:false
    ...
    const [createUserWithEmailAndPassword,user,loading,error,] = useCreateUserWithEmailAndPassword(auth);
                            // Login  
    const [signInWithEmailAndPassword,Signinuser,Singinloading,Singerror,] = useSignInWithEmailAndPassword(auth); 
                           //Signin with Goolge
...
const GetDocumentData = async () => {
  try {
    ...
    return(
        <EcomContext.Provider value={{isSideBar,setIsSideBar,search,setSearch,isScreen,setScreen,isLoggedIn,
          Products,setProducts,GetDocumentData,StoreAllImages,AddItemsToCart,UpdatePlusToCart,
          UpdateMinusToCart,cartItem,calculateTotalPrice,FirebaseData,setFirebaseData,AddItemsToWish,WishItem,
          RemoveItemsToWish,RemoveItemsToCart,setCartItem,isWish,setIsWish,TotalPrice,setTotalPrice,
          BuyItem,setBuyItem,setCurrentUser,currentUser,first,setFirst,
          createUserWithEmailAndPassword,user,loading,error,AddDataInDB,signInWithGoogle,
          signInWithEmailAndPassword,Signinuser,Singinloading,Singerror,
          signOut, SignOutloading, SingOuterror,isLoggedIn,AddDataOrderInDB,input,setInput
        }}>
            {children}
