...
const BuyNow = () => {
  ...
  const calculateTotalPrice = () => {
    setTotal(
      BuyItem?.reduce((total, item) => total + item.price * item.qty, 0)
    );
  };

  const handlePlaceOrder = async (e) => {
    const orderDetails = BuyItem.map((item) => {
      return `${item.title} - Qty: ${item.qty} - Price: $${
        item.price * item.qty
      }`;
    }).join("\n");
    const OrderDetails = {
  ...
  return (
    <>
      <div
        className="flex flex-col items-center lg:items-start lg:flex-row justify-between 
      space-x-4 lg:space-x-0 w-full lg:w-[90%] relative left-[50%] translate-x-[-50%] p-5"
      >
        <form className="flex flex-col items-start space-y-6 border-b-2 lg:border-none pb-5">
          <h1 className="text-black text-3xl lg:text-5xl font-medium">
            Fill Out The Form
          </h1>
          <div className="flex flex-col space-y-1 w-[300px]">
            <span className="text-gray-600 text-md">Full Name</span>
            <input
              type="text"
              required
     ...
     outline-none text-black"
          ...
          <div className="flex flex-col space-y-1 w-[300px]">
            <span className="text-gray-600 text-md">Email Address</span>
            <input
              type="email"
              required
              className="p-2 bg-slate-300 rounded-sm
     outline-none text-black"
          ...
          <div className="flex flex-col space-y-1 w-[300px]">
            <span className="text-gray-600 text-md">Street Address</span>
            <textarea
              required
              className="p-2 bg-slate-300 rounded-sm
     outline-none text-black"
     ...
     outline-none text-black"
          ...
          <div className="flex flex-col space-y-1 w-[300px]">
            <span className="text-gray-600 text-md">Town/City</span>
            <input
              type="text"
              required
              className="p-2 bg-slate-300 rounded-sm
     outline-none text-black"
          ...
          <div className="flex flex-col space-y-1 w-[300px]">
            <span className="text-gray-600 text-md">Phone Number</span>
            <input
              type="tel"
              required
              className="p-2 bg-slate-300 rounded-sm
     outline-none text-black"
        ...
        <div
          className="flex flex-col font-medium  space-y-4 pb-5 w-full  lg:w-[45%]  
     mt-14  lg:text-2xl"
        >
          ...
          <button
            className="bg-red-700 px-3 py-2  text-white font-normal text-sm w-[min-content]
    whitespace-nowrap ml-5"
